...
export function registerRemindActionItems(worker: Worker) {
  worker.tool("remindActionItems", {
    ...
    description:
      "Read the Action Items database and return items needing attention — overdue, due soon, unassigned, missing a due date, or with Review Status unchecked. Use when the user asks what is overdue, what to do today, or for standup reminders.",
    schema: j.object({
      daysAhead: j
        .number()
        .nullable()
        .describe(
          `Days ahead to flag as 'due soon'. Default ${DEFAULT_DAYS_AHEAD}. Pass null for default.`,
        ),
    }),
    ...
    execute: async ({ daysAhead }, context: any) => {
      ...
      if (!dbId) {
        throw new Error(
          "ACTION_ITEMS_DB_ID is not set. Add it via `ntn workers env set ACTION_ITEMS_DB_ID=<id>` or .env.",
        );
      }
      ...
      for (const c of categorized) {
        for (let i = 0; i < c.categories.length; i++) {
          ...
          const reason = c.reasons[i] ?? c.reasons[0] ?? "";
          buckets[cat].push(toReminderItem(c, reason));
        }
      }
